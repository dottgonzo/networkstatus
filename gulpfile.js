...
gulp.task('release', function (done) {
    ...
    exec('rsync -av ' + __dirname + '/* /tmp/' + utime + ' --exclude dists --exclude gulpfile.js --exclude typings --exclude ".git*" --exclude node_modules --exclude "tsconfig.json" --exclude "*.ts" && cd /tmp/' + utime + '/front && npm i --production --silent && cd /tmp/' + utime + ' && npm i --production --silent && rm -rf ' + __dirname + '/dists/' + os.arch() + ' && mv /tmp/' + utime + ' ' + __dirname + '/dists/' + os.arch()+' && cd ' + __dirname + '/dists/' + os.arch()+' && find . -name "*.ts" -delete && rm -rf $(find . -name "typings" -print) && rm -rf .git*', function (err, stdout, stderr) {
        if (err) {
            console.log(err)
        }
        done()
